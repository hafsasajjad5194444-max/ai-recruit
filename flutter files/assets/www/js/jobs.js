...
function initializeFilters() {
    // Delegate filter UI population and wiring to main.js implementation
...
function populateFilterOptions() {
    ...
    typeSelect.innerHTML = `<option value="all">All Types</option>` +
        types.map(type => `<option value="${type}">${formatLabel(type)}</option>`).join('');
}
...
function applyFilters() {
    // If main.js provides a shared filter function, use that.
    if (typeof applyFiltersFromControls === 'function') {
